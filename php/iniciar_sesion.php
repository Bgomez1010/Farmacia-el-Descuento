<?php
    ...
    if($check_user->rowCount()==1){
    	...
    	if($check_user['usuario_usuario']==$usuario && password_verify($clave, $check_user['usuario_clave'])){
            ...
            if($check_user['id_cargo']==2){
                if(headers_sent()){
                    echo "<script> window.location.href='index.php?vista=home_empleado'; </script>";
                }else{
                    header("Location: index.php?vista=home_empleado");
		    	
                }
            }
            if($check_user['id_cargo']==3){
                if(headers_sent()){
